...
// M14 机构榜独立页（自趋势页抽出，与论文列表/作者榜/个人画像并列）：TOP 50/100/全部 + 服务端分页 + 搜索防抖
import { onMounted, ref, watch, type Ref } from 'vue'
import { useRouter } from 'vue-router'
import { getInstitutionsTop } from '../api/stats'
import type { InstitutionStat } from '../types'
...
const page = ref(1)
const total = ref(0)
const items = ref<InstitutionStat[]>([])
const search = ref('')
...
async function load() {
  page.value = 1
  try {
    const p = await getInstitutionsTop(1, pageSize(), search.value.trim() || undefined)
    items.value = p.items
    total.value = p.total
  } catch (e) {
...
async function loadPage() {
  try {
    const p = await getInstitutionsTop(page.value, PAGE_SIZE, search.value.trim() || undefined)
    items.value = p.items
    total.value = p.total
  } catch (e) {
