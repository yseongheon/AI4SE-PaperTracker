...
| **user_marks** | user_id(FK), paper_id(FK), mark_type(bookmark/read/read_later), created_at | M6+M9 个性化阅读标记。复合主键 (user_id, paper_id, mark_type) 幂等；M9 迁移旧数据归首个注册用户 |
...
| GET | /api/stats/words | 词云（M7）：`limit, scope=all\|ai4se`，摘要英文停用词过滤+词频 |
| GET | /api/stats/authors | 作者榜（M13 全量分页）：`page, page_size, q`（`q` 作者名模糊搜索，不区分大小写），返回 `{items, total, page, page_size}`；每项论文数/AI4SE 数/主要主题/机构 |
| GET | /api/stats/institutions | 机构榜（M13 全量分页）：`page, page_size, q`（`q` 机构名模糊搜索，不区分大小写），返回 `{items, total, page, page_size}`；每项论文数/AI4SE 数/主要主题 |
...
### 生产部署（M10：阿里云服务器，课题组正式环境）

**架构**（阿里云 ECS Ubuntu 22.04，2C1.6G）：

```text
公网 :80 (nginx 反代，安全组已放行) → 127.0.0.1:8000 (systemd: papertracker.service)
```

| 项 | 说明 |
|---|---|
| 访问地址 | `http://8.166.134.107/`（80 端口复用旧网站放行规则，**无需改安全组**；安全组仅放行 22/80/443，8000 公网不可达） |
| 代码位置 | `/opt/ai4se-papertracker/`（本地打包 backend+frontend/dist+data 上传，uv 管理） |
| 服务 | `systemd` 托管：`papertracker.service`（开机自启、崩溃自动重启） |
| 定时任务 | APScheduler 进程内（每日 09:30 爬取、周五周报），UTC 调度无时区问题 |
| 数据库 | `/opt/ai4se-papertracker/data/papers.db`（SQLite，每日爬取增量写入） |
| nginx | `/etc/nginx/conf.d/starwise.conf` 反代 8000；原 starwise(8080) 备份在 `starwise.conf.bat`（一行 proxy_pass 可回滚） |
...
**运维**：`systemctl status papertracker` 查状态；`tail -f /root/citations.log` 查引用回填日志；数据备份 = 定期拷贝 `/opt/ai4se-papertracker/data/`；服务器 root 登录已配置 SSH 密钥（本机 `id_ed25519` 免密）。

## 12. 常用任务手册

| 任务 | 命令 |
|---|---|
| 一键启动（课题组） | 双击根目录 `start.bat`（自动起前后端+开浏览器） |
...
| 生成个人周报（M9） | `python -m scripts.personal_report`（每周五自动为收藏数>0 的用户生成，输出 data/reports/personal/） |
...
| **M8 课题组化 P1** | 引用数回填（DR-026：Crossref+S2 双源）+ 被引展示/排序/最低被引筛选 + 导出含被引（DR-023 原方案 OpenAlex 被墙，已替换） | pytest 全绿；回填完成、排序筛选生效 | ⏳ 进行中（2026-08-20，代码完成+128 测试全绿；回填移至**服务器生产库**后台运行，约 3 小时） |
| **M9 课题组化 P2** | 登录注册（DR-027：自建 HMAC token + pbkdf2）+ 标记用户隔离 + 个人周报/画像（DR-028）；旧标记归首个注册用户 | pytest 全绿；注册→登录→收藏互不可见；未登录可浏览、标记 401；个人周报生成 | ✅ 完成（2026-08-19） |
| **M10 生产部署** | 阿里云 ECS 部署（Ubuntu 22.04，2C1.6G）：uv 装 Python 3.14 + 依赖、systemd 常驻、nginx 80 反代（避开安全组未放行 8000 的问题）、数据全量迁移、服务器端引用数回填 | 公网 <http://8.166.134.107/> 访问正常（页面/API/静态资源/SPA 路由全通）；每日自动爬取在服务器运行 | ✅ 完成（2026-08-20） |

**当前进度**：M0~M3 已完成（2026-08-17）。M1 验证记录：4 个 A 会 stream 全量拉取 16967 条（ICSE 7894 / sigsoft 3739 / kbse 3776 / issta 1558，缓存 7 天）；arXiv 近 3 天 4 篇幂等（重跑 new=0/updated=4）；匹配器用真实 DBLP 记录端到端验证通过。M2 验证记录：49 条关键词规则初筛 3875 篇 → 2503 候选；DeepSeek 精标 2501 篇全部完成、**0 失败、成本 $0.65**（deepseek-chat，limit $5）；确认 AI4SE **2233 篇**（58%）；主题分布（llm 标签）：llm4se_general 1761 / testing 803 / code_generation 750 / analysis 670 / defect_detection 532 / code_repair 314 / requirements 135 / code_summarization 78 / code_translation 61 / other 51，1986 篇多主题；抽样 15 篇质量验证通过。M3 验证记录：61 pytest 全绿（含 17 个 API 集成测试，TestClient+内存库）；curl 真实验证全部端点——列表（分页/搜索/主题/会议/年份/AI4SE 过滤、newest/venue 排序）、详情（作者/标签/中文摘要/双链接/404）、topics 计数（llm4se_general 1761）、venues 计数（FSE 66 / ASE 5 / ICSE 3）、trends 三分组（topic 10 条线按天零填充 / venue 3 线 / year 2 年）。M4 验证记录（用户拍板：el-table 表格 + 左侧筛选侧栏 + 多页签）：`vue-tsc` 严格模式 + `vite build` 一次通过；前后端冒烟验证经 dev 代理（localhost:5173 → 127.0.0.1:8000）全部端点可达（列表真实数据/详情/主题计数/会议计数/趋势按天序列）；trends 周/月聚合在前端完成（DR-020 方案 A，见 utils/trends.ts，年份分组恒按原样展示）；年份筛选选项取自趋势接口年份分组（数据驱动）。M5 验证记录（2026-08-18，用户拍板 A②+B②+C①+D②）：① A 匹配歧义复核——matcher 在 pending 时把多候选快照写入 papers.match_candidates（JSON 新列，migration 7c8d79d4ca91），新增 scripts/review_pending.py（list/accept --idx|--key/reject，venue 按 dblp_key 前缀反查）；现有 pending=0（matched 74 全部定案）；71 pytest 全绿（+10 复核脚本测试）；② C README 访客版完成；③ D 增量验证启动——08-18 首跑成功：arXiv 429 退避重试成功、DBLP 4 stream 全缓存命中零 API 请求、匹配 2826 条 matched=0/pending=0、幂等 new=0/updated=1，观察期至 08-25。M6 验证记录（2026-08-19，用户拍板：AI 阅读增强+个性化阅读+周报+导出四包）：migration 28cc6f44279b（papers.highlights JSON + user_marks 复合主键表）；classifier 输出扩展 highlights（容错解析不破坏旧格式）；101 pytest 全绿（+30：highlights 解析/related 排序、marks API 幂等与过滤、三格式导出含 BOM/转义/作者格式、周报统计与渲染）；vue-tsc+vite build 通过；curl 冒烟全通——marks toggle/列表 marks 集合/bookmark 过滤/详情 related 5 篇/CSV BOM/BibTeX 条目/JSON（修了 model_dump 的 date 序列化 bug）/周报 2026-W34.md 生成；亮点速读回填完成（2026-08-19：2234/2235 成功、1 失败，成本 $0.79、累计 $1.44，2212 篇 highlights 落库——注意副产物：22 篇在重判中被降级为非 AI4SE，属 LLM 判定波动，1% 级）。M7 验证记录（2026-08-19，DR-023/024/025 拍板）：migration 58e2e53bd581（papers.deep_summary）；118 pytest 全绿（+17：词云停用词/作者榜聚合/交叉矩阵/共著边权重/author+field+年份区间过滤/单篇 bibtex/pdf_url/export ids/deep-summary 缓存语义——真实 llm_cost.json 累计超测试 limit 致 test_cost_limit_raises 失败，已补 _state_file 隔离）；vue-tsc+build 通过；curl 冒烟全通——words（code 4204/llm 2379）、authors（David Lo 43 篇榜首）、cross 3×7 矩阵、coauthor 节点边、author=david lo 过滤 43 篇、pdf_url、单篇 bibtex、export ids；前端 8 页签趋势页（图例单击只看/再击恢复——纯响应式 legendSelected 无实例依赖）、饼图/堆叠/热力/合作网络（TOP100 共著）/词云（echarts-wordcloud 副作用注册兼容 echarts 6，--legacy-peer-deps 安装）/作者榜（点击跳列表过滤）、列表多选导出+高级筛选 popover+作者点击、详情 AI 深度摘要按钮+复制 BibTeX+PDF 直达+作者点击、start.bat。M9 验证记录（2026-08-19，DR-027/028 拍板）：migration 8e07230e0ec0（users 表 + user_marks 加 user_id 复合主键——**SQLite 显式重建表**：batch_alter_table 的 drop_constraint 反射不到 SQLite 主键约束名、batch 内 execute 在列未添加时执行，两坑均踩过并规避）；147 pytest 全绿（+19 认证/隔离/画像/周报；适配 marks 需登录）；前端登录注册页+authStore(localStorage)+axios 拦截器+路由守卫+导航用户区+个人画像页（统计/主题分布条/最近收藏）；curl 冒烟：注册（xiaoyu id=1 承接旧标记）→登录→未登录 401→收藏→画像 counts/recent/topics 正常；个人周报脚本手动可跑（无收藏用户跳过）；M8 引用数回填仍在后台（约 3 小时，完成自动通知）。M10 部署验证记录（2026-08-20）：阿里云 ECS（Ubuntu 22.04，2C1.6G，Python 3.14.7 经 uv 安装）——tar 打包 backend+dist+data 上传 /opt/ai4se-papertracker；**坑 1**：pyproject 无 [build-system] 且 flat-layout 多顶层包致 setuptools 构建失败 → 补 build-system + [tool.setuptools.packages.find] include=["app*"]；**坑 2**：阿里云安全组未放行 8000（控制台登录需短信验证码不可代操作、实例无 RAM 角色）→ 复用旧网站已放行的 80 端口：nginx 反代 8000（starwise.conf 备份可回滚），**公网 <http://8.166.134.107/> 全通**（首页/API/静态资源/SPA 路由）；systemd papertracker.service 常驻（内存 ~82M）；**数据线**：服务器为生产库（08-20 自动爬取新增 43 篇至 3918），引用数回填改在服务器后台跑（nohup run_citations，幂等跳过已回填，日志 /root/citations.log）；旧部署残骸 /root/AI4SE-PaperTracker（8010 端口、无数据）已停；服务器端 root 已配 SSH 密钥免密；M7 高级筛选 popover+tooltip 嵌套点不开 → 改 el-dialog 弹窗+已应用条件摘要。运行方式：后端 `python -m uv run uvicorn app.main:app --reload --port 8000`（backend/ 下，uvicorn 不全局安装；bash 中 PATH 的 python 可能是 .venv 的，此时用 `.venv/Scripts/python.exe -m uvicorn` 更稳）；前端 `npm run dev`（frontend/ 下，proxy 到 8000；注意 vite 默认绑定 IPv6 ::1，浏览器用 localhost 而非 127.0.0.1 访问）。M13 验证记录（2026-08-21，榜单全量分页）：`/api/stats/authors` 与 `/api/stats/institutions` 移除 `limit` 上限，改 `page/page_size` 返回 `{items,total,page,page_size}`（作者 12,484 / 机构过滤后 248）；前端趋势页作者·机构榜选择器 TOP 50/TOP 100/全部（默认全部，服务端分页）+ 双独立分页器 + 序号列全局名次；186 pytest 全绿（+8：作者分页切片/越界页/机构 total 过滤/API 分页与 422）；vue-tsc+vite build 通过；附注：`paper_authors` 实际唯一约束是 `(paper_id, author_id)`（非 position），`ai4se_count` 改 DISTINCT 为与机构榜口径一致（防御性加固）；`deploy/deploy_server.py` 健康检查 curl 已适配 page/page_size；作者/机构榜新增 `q` 模糊搜索（前端搜索框防抖 300ms，搜索时强制走分页以便浏览全部命中）。**导出按钮 bug 修复（2026-08-21）**：前端 `exportUrl` 少了 `/api` 前缀（返回 `/export?...`），点「导出筛选结果/导出选中」会命中 SPA 兜底返回首页 HTML、无下载效果——已改 `frontend/src/api/papers.ts` 为 `/api/export?...`（生产与 dev 双模式均可用，curl 验证三种格式 Content-Disposition 正常）。
